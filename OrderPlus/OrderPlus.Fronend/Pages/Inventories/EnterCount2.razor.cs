...
namespace OrderPlus.Fronend.Pages.Inventories
{
    public partial class EnterCount2
    {
        ...
        private void SaveCount()
        {
            ...
            snackbar.Add("Changes saved successfully", Severity.Error);
        }
        ...
        private ActionResponse<bool> ValidateCount()
        {
            foreach (var item in inventoryDetails!)
            {
                if(item.Count2<0 || item.Cost <0 )
                {
                    return new ActionResponse<bool>
                    {
                        WasSuccess = false,
                        Message=$"The product: {item.Product!.Name}, has cost: {item.Cost} " +
                        $"and count: {item.Count2}. Negative values ​​are not allowed."
                    };
                }
                if (item.Count2 != 0 && item.Cost <= 0)
                {
                    return new ActionResponse<bool>
                    {
                        ...
                        $" has cost 0 and a quantity entered in the count of: {item.Count2}, " +
        ...
        private async Task FinishCountAsync()
        {
            var parameters = new DialogParameters
            {
                { "Message", "Are you sure you want to end countdown #2?" }
            };
            var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.ExtraSmall, CloseOnEscapeKey = true };
            ...
            snackbar.Add("Count #2 closed, you may proceed to count #3.", Severity.Success);
